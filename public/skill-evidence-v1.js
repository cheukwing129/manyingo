...
function update(previous,answer,question,skillId,now){
 ...
 return{version:VERSION,eligibleAttempts:(prior.version===VERSION?prior.eligibleAttempts:0)+1,correctCount:(prior.version===VERSION?prior.correctCount:0)+(answer&&answer.isCorrect?1:0),questionIds,contextIds,sessionDates,firstAnsweredAt:first,lastAnsweredAt:at,delayedCorrectAt:prior.version===VERSION&&prior.delayedCorrectAt?prior.delayedCorrectAt:delayed?at:null};
}
function assess(record){
 const source=record&&typeof record==='object'?record:{},evidence=normalize(source.evidence),checks={version:evidence.version===VERSION,mastery:Number(source.mastery)>=MIN_MASTERY,attempts:evidence.eligibleAttempts>=MIN_ATTEMPTS,questions:evidence.questionIds.length>=MIN_QUESTIONS,contexts:evidence.contextIds.length>=MIN_CONTEXTS,sessions:evidence.sessionDates.length>=MIN_SESSIONS,delayed:!!evidence.delayedCorrectAt,recentCorrect:source.lastCorrect===true},verified=Object.values(checks).every(Boolean);
...
return{VERSION,QUESTION_CONTRACT_VERSION,MIN_MASTERY,MIN_ATTEMPTS,MIN_QUESTIONS,MIN_CONTEXTS,MIN_SESSIONS,DELAY_MS,unique,timeValue,contextId,eligibleQuestion,empty,normalize,update,assess};
});
