...
function reviewAnswer(value){const text=String(value||'').trim();return text?'參考答案：'+text+(/[。！？!?]$/.test(text)?' ':'。 '):'';}
...
return{STORAGE_KEY,SKILLS,skillId,isStage3Question,challengeQuestions,buildChallenge,validPassageSet,buildPassageSet,reviewAnswer,highlightedPassage,answerCorrect,summarizeResults,install,renderLanding,startPassageSet,startChallenge};
});
