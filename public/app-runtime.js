...
function hasPendingGoogleRedirect(){try{return Number(sessionStorage.getItem('manyingo_google_redirect_pending_v1'))>0}catch(_){return false}}
function redirectMessage(fb,state){if(state&&state.google)return'';const issue=fb&&typeof fb.getGoogleRedirectIssue==='function'?fb.getGoogleRedirectIssue():null;return issue&&issue.message?issue.message:''}
...
async function login(){if(busy)return;busy=true;render(null);let fb,errorMessage='';try{fb=await firebase();render(await fb.getAccountState());const manager=sync();const result=await fb.signInWithGoogle();if(result&&result.redirecting)return;if(manager&&typeof manager.syncNow==='function')await manager.syncNow()}catch(error){const code=String(error&&error.code||'');errorMessage=code==='auth/operation-not-allowed'?'Google 登入尚未在 Firebase 啟用':code==='auth/unauthorized-domain'?'目前網域尚未加入 Firebase 授權':'Google 登入失敗，請稍後再試'}finally{busy=false;if(fb){const state=await fb.getAccountState().catch(()=>null);render(state,errorMessage||redirectMessage(fb,state))}else if(errorMessage)render(null,errorMessage)}}
...
async function install(){installStyle();if(!host())return false;try{const fb=await firebase();const state=await fb.getAccountState();render(state,redirectMessage(fb,state));void fb.onAccountChanged(next=>render(next,redirectMessage(fb,next)));window.addEventListener('manjingo:account-sync-state',event=>{lastSync=event&&event.detail&&event.detail.status||'';void fb.getAccountState().then(next=>render(next,redirectMessage(fb,next)))})}catch(error){render(null,'目前使用本機模式')}return true}
function studyActive(){return !!(document.body&&document.body.classList&&document.body.classList.contains('study-focus'))}
function scheduleInstall(){installStyle();render(null);const pendingRedirect=hasPendingGoogleRedirect();const ready=()=>{if(studyActive()){setTimeout(ready,1200);return}const run=()=>{if(studyActive()){setTimeout(ready,1200);return}void install()};if(pendingRedirect){setTimeout(run,0);return}if(typeof window.requestIdleCallback==='function')window.requestIdleCallback(run,{timeout:1200});else setTimeout(run,150)};ready()}
