...
function studyActive(){return !!(document.body&&document.body.classList&&document.body.classList.contains('study-focus'))}
function scheduleInstall(){installStyle();render(null);const pendingRedirect=hasPendingGoogleRedirect();const ready=()=>{if(studyActive()){setTimeout(ready,1200);return}const run=()=>{if(studyActive()){setTimeout(ready,1200);return}void install()};if(pendingRedirect){setTimeout(run,0);return}if(typeof window.requestIdleCallback==='function')window.requestIdleCallback(run,{timeout:1200});else setTimeout(run,150)};setTimeout(ready,pendingRedirect?0:4000)}
