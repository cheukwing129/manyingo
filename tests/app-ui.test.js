...
test('homepage and lesson share one UI foundation',()=>{
  ...
  assert.match(shell,/function installStartupRuntimeDeferral\(\)/);
  assert.match(shell,/function install\(\)\{if\(shellInstalled\)return false;shellInstalled=true;installStartupRuntimeDeferral\(\);installUiFoundation\(\);enhancePlayerStatus\(\);enhanceAppNav\(\)/);
